...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do while (icmd /= 0)
        ...
        !if (ob(icmd)%rcv_tot > 0) then
          ...
          ob(icmd)%hin_lat = hz
          ob(icmd)%hin_til = hz
          ob(icmd)%hin_aqu = hz   !!EBroute_revap
          ht1 = hz
          ...
          if (ob(icmd)%typ == "hru" .or. ob(icmd)%typ == "ru") then  !only convert hru and subbasin hyds for routing
            ...
            !else
              conv = ob(icmd)%area_ha * 10.  !!EB comme v60.5.4
            !end if
      ...
      if (time%yrs > pco%nyskip .and. time%step == 0) then
        ...
        if (pco%sd_chan%nb_print == 0) then
          ...
          do j = 1, pco%sd_chan%nb_print   !!EB affichage leger
            call sd_chanmorph_output ( pco%sd_chan%list_print(j) )
            call sd_channel_output ( pco%sd_chan%list_print(j) )
            if (cs_db%num_tot > 0) then 
              call cha_pesticide_output ( pco%sd_chan%list_print(j) )   
              !call ch_pathogen_output ( pco%sd_chan%list_print(j) )
            end if
          end do
        end if     
