    ...
    !! lsu
     ...
     !! channel-lte
        ...
        read (107,*,iostat=eof) name, pco%sd_chan%d,pco%sd_chan%m,pco%sd_chan%y,pco%sd_chan%a,pco%sd_chan%nb_print   !!EB affichage leger
        if (pco%sd_chan%nb_print>0) then
          backspace (107)
          allocate (pco%sd_chan%list_print(pco%sd_chan%nb_print))
          read (107,*,iostat=eof) name, pco%sd_chan%d,pco%sd_chan%m,pco%sd_chan%y,pco%sd_chan%a, &
                                   pco%sd_chan%nb_print, (pco%sd_chan%list_print(ii), ii=1,pco%sd_chan%nb_print)
        end if
        if (eof < 0) exit          
     !! aquifer
