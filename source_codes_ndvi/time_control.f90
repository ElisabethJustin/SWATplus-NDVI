...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
    ...
    !1235 format (1x, a, 2x, i4)
        ...
        do julian_day = time%day_start, time%day_end_yr      !! begin daily loop
          ...
          if (time%day == 1) then
            write (*,1234) cal_sim, time%yrc, time%yrs, time%yrc_tot,  &
                   date_time(5), date_time(6), date_time(7)
            !do j=1,pcom(50000)%npl
            !  write (*,*) '  EB0509 time_control ',pcomdb(pcom(50000)%pcomdb)%pl(j)%cpnm, pcom(50000)%plcur(j)%gro
            !end do
                   
            !!EBndvi deb re-initialisation du "util" puis lecture du NDVI pour la nouvelle annee
            do j = 1, sp_ob%hru
              hru(j)%ndvi%util = .false.
            end do
            call ndvi_read (time%yrc)
            !!EBndvi fin
          ...
          ! near winter solstice (winter solstice is around June 22)
      ...
      return
 1234 format (1x, a, i4, 2x, ' Yr ', i3,' of ', i3, 2x, " Time",2x,i2,":",i2,":",i2)
