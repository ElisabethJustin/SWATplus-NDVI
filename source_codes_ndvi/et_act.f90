...
!!    real, parameter :: esd = 500., etco = 0.80, effnup = 0.1
      ...
      !!EBndvi deb
      real :: tsat                             !|humidite a saturation
      real :: tmid                             !|humidite quand Kr=1/2
      real :: p_merlin                         !|exposant dans la loi de Merlin pour Kr
      real :: Kr                               !|coeff de stress pour l'evaporation
      !!EBndvi fin                              
...
!! CN methods, canstor will always equal zero.
      ...
      if (pet > 1.0e-6) then
        ...
        if (bsn_cc%pet /= 1) then   !! EB pourquoi c'etait commente ?
        
          if (hru(j)%ndvi%util) then
            ep_max = pet_day * (1.464 * hru(j)%ndvi%ndvi_j - 0.253)  !!EBndvi calcul du Kcb-NDVI Toureiro 2016
            !ep_max = pet_day * (1.494 * hru(j)%ndvi%ndvi_j - 0.143)  !!EBndvi   FAO-S2
          else
        ...
        if (hru(j)%sno_mm >= 0.5) then
          ...
          if (hru(j)%ndvi%util) then
            !!eaj = 1  - (1.15 * hru(j)%ndvi%ndvi_j - 0.15)   !!EBndvi  calcul du Fcover-NDVI
            eaj = 1  - (1.25 * hru(j)%ndvi%ndvi_j - 0.13)   !!EBndvi  Saadi 2015
          else
...
!! take soil evap from each soil layer
      ...
      do ly = 1, soil(j)%nly
        ...
        if (dep < esd) then
          ...
          !!EB3001 calcul Kr de 2 menieres differentes (SWAT classique VS FAO)
          if (.not.(hru(j)%ndvi%util)) then
            ...
            tsat = 0.489 - 0.126*soil(j)%phys(ly)%sand
            tmid = 0.2 + 0.28*soil(j)%phys(ly)%clay - 0.16*soil(j)%phys(ly)%sand
            p_merlin = log(0.5) / (log(0.5) - 0.5*cos(3.14159 * tmid / tsat))
            if (tsat > soil(j)%phys(ly)%st/soil(j)%phys(ly)%thick + soil(j)%phys(ly)%wp) then
              Kr = ( (1 - cos(3.14159 * (soil(j)%phys(ly)%st/soil(j)%phys(ly)%thick + soil(j)%phys(ly)%wp) / tsat)) / 2 ) ** p_merlin
            else
              Kr = 1.
            end if 
            sev = sev * Kr
