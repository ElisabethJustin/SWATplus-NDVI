      ...
      if (pldb(idp)%typ == "warm_annual" .or. pldb(idp)%typ == "cold_annual" .or.  &
        ...
        if (hru(j)%ndvi%util) then   !!EBndvi faire pousser les racines en fonction du NDVI comme dans samir
            pcom(j)%plg(ipl)%root_dep = 10. + (pldb(idp)%rdmx*1000. - 10.) * (1.25 * hru(j)%ndvi%ndvi_j - 0.13)/0.9   !!EBndvi  en fonction du Fcover-NDVI
        else     
