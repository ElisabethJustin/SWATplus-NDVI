      ...
      if (.not. i_exist .or. in_cli%tmp_cli == "null") then
        write (*,*) '    Attention fichier tmp.cli pas trouvé !' !!EB
         allocate (tmp(0:0))
         allocate (tmp_n(0))
      else
      do 
...
!!!!!weather path code
       if (in_path_tmp%tmp == "null") then
         if (i==1)  write (*,*) '   ... lecture fichier température ',tmp(i)%filename, ' etc' !!EB
         open (108,file = tmp(i)%filename)
       else
         if (i==1)  write (*,*) '   ... lecture meteo dans le dossier ',in_path_tmp%tmp !!EB
