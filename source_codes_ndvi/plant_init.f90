      ...
      use hru_module, only : cn2, cvm_com, hru, ihru, isol, rsdco_plcom, ipl
      use soil_module
      use plant_module
...
!      use hru_lte_module
      ...
      !! allocate plants
        ...
        do ipl = 1, pcom(j)%npl
          ...
          !!EB pcom(j)%plcur(ipl)%phuacc = pcomdb(icom)%pl(ipl)%phuacc
          ...
          !!EB pcom(j)%plg(ipl)%lai = pcomdb(icom)%pl(ipl)%lai
          pcom(j)%laimx_sum = pcom(j)%laimx_sum + pldb(idp)%blai
          !!EB pl_mass(j)%tot(ipl)%m = pcomdb(icom)%pl(ipl)%bioms
          !!EB pcom(j)%plcur(ipl)%curyr_mat = int (pcomdb(icom)%pl(ipl)%fr_yrmat * float(pldb(idp)%mat_yrs))
