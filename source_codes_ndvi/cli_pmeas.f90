      ...
      if (.not. i_exist .or. in_cli%pcp_cli == "null") then
        write (*,*) '    Attention fichier pcp.cli pas trouvé !' !!EB
        allocate (pcp(0:0))
        allocate (pcp_n(0))
      else
      do 
      ...
      if (in_path_pcp%pcp == "null") then 
        if (i==1)  write (*,*) '   ... lecture fichier pluie ',pcp(i)%filename, ' etc' !!EB
        open (108,file = pcp(i)%filename)
      else
        if (i==1)  write (*,*) '   ... lecture meteo dans le dossier ',in_path_pcp%pcp !!EB
