      ...
      do ires = 1, sp_ob%res
        ...
        do isp_ini = 1, db_mx%res_init
          if (res_dat_c(ires)%init == res_init_dat_c(isp_ini)%init) then
            ...
            !! initial volume lu dans initial.res !!EB
            select case (res_init_dat_c(isp_ini)%volinicode)
              case("zero")
                res(ires)%flo = 0.
              case("pvol")
                res(ires)%flo = res_ob(ires)%pvol * res_init_dat_c(isp_ini)%volini
              case("evol")
                res(ires)%flo = res_ob(ires)%evol * res_init_dat_c(isp_ini)%volini
              case("meas")
                res(ires)%flo = res_init_dat_c(isp_ini)%volini
            end select
          end if
        end do

        call res_convert_mass (res(ires), 1.)  !!EB ce n'est pas pratique... res_ob(ires)%pvol)
