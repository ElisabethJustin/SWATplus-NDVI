      ...
      use hru_module, only : hru, ihru, cn2, phubase, ndeat, igrz, grz_days,    &
        ...
        fertorgp, fertsolp, sweepeff, yr_skip  !!EB ipl
      use soil_module
      use plant_module
      use time_module
      ...
      if (mgt%op /= "fert      ") then
        ...
        icom = pcom(j)%pcomdb
        if (icom > 0) then
          if (pcom(j)%npl > 0) then  !!EB 1 remplacé par 0 (si npl=1 mais qu'il y a bcp de mgtops, il ne faut pas toutes les faire)
            do ipl = 1, pcom(j)%npl
      ...
      select case (mgt%op)
          ...
          case ("plun")    !!EB plant the only plant growing this rotation year
            icom = pcom(j)%pcomdb
            pcom(j)%days_plant = 1       !reset days since last plant
            ...
            idp = pcomdb(icom)%pl(ipl)%db_num
            if (mgt%op_char == pcomdb(icom)%pl(ipl)%cpnm) then
              if (j==50000) write (*,*) '    Jour',time%day,' plant ',mgt%op_char  !!EBtext
              do jj = 1, pcom(j)%npl
                call mgt_killop (j, jj)   
              end do
          ...
          case ("harv")  !! harvest only operation
            ...
            ipl = mod(pcom(j)%rot_yr-1,pcom(j)%npl)+1
              ...
              if (mgt%op_char == pcomdb(icom)%pl(ipl)%cpnm .or. mgt%op_char == "all") then
                ...
                if (j==50000) write (*,*) '    Jour',time%day,' harvest ',mgt%op_char  !!EBtext  
                ...
                pcom(j)%plcur(ipl)%phuacc = 0.  !!EB deplacé
              ...
              !!EB dans la boucle 'if' pcom(j)%plcur(ipl)%phuacc = 0.
              end if
            !!EB end do
            ...
            case ("kill")   !! kill operation
              do ipl = 1, pcom(j)%npl
                ...
                if (mgt%op_char == pcomdb(icom)%pl(ipl)%cpnm .or. mgt%op_char == "all") then
                  ...
                  pcom(j)%plcur(ipl)%phuacc = 0.  !!EB deplacé rq inutile si c'est deja fait dans killop...
          ...
          case ("hvkl")   !! harvest and kill operation
            ...
            ipl = mod(pcom(j)%rot_yr-1,pcom(j)%npl)+1
              ...
              if (mgt%op_char == pcomdb(icom)%pl(ipl)%cpnm .or. mgt%op_char == "all") then
                ...
                pcom(j)%plstr(ipl) = plstrz  !!EB deplacé rq inutile si deja dans killop...
                pcom(j)%plcur(ipl)%phuacc = 0.
                phubase(j) = 0.
              ...
              !!EB dans la boucle 'if' pcom(j)%plstr(ipl) = plstrz
              !!pcom(j)%plcur(ipl)%phuacc = 0.
              !!phubase(j) = 0.
            !!EB end do
