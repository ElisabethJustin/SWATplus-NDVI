...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use hru_module, only : ep_max, epmax, htfac, ihru, ipl, par, translt, hru !!EBndvi ajout 'hru' 
      use soil_module
      use plant_module
      ...
      do ipl = 1, pcom(j)%npl
        if (pcom(j)%lai_sum > 1.e-6) then
          ...
          epmax(ipl) = 0.
          if (hru(j)%ndvi%util) then  !!EBndvi calcul de transpiration même si la culture principale ne pousse pas encore
            epmax(1) = ep_max
          end if
        end if
      end do
      ...
      if (pcom(j)%npl>0 .and. (npl_gro==0 .and. hru(j)%ndvi%util)) then  !!EBndvi calcul de transpiration même si la culture principale ne pousse pas encore
        ipl=1
        call pl_waterup
      end if
