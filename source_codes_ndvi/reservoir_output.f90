...
!!!!! daily print
         if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%res%d == "y") then
            !!EBpetitout write (2540,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, res_wat_d(j), res(j), &
                !!res_in_d(j), res_out_d(j)
               write (2540,101) time%mo, time%day_mo, time%yrc, ob(iob)%name, res_wat_d(j), res(j)%flo, res_in_d(j)%flo, res_out_d(j)%flo
...
!!!!! monthly print
        if (time%end_mo == 1) then
          ...
          if (pco%res%m == "y") then
            write (2541,101) time%mo, time%day_mo, time%yrc, ob(iob)%name, res_wat_m(j), res(j)%flo, res_in_m(j)%flo, res_out_m(j)%flo !!EB
              if (pco%csvout == "y") then
...
!!!!! yearly print
       if (time%end_yr == 1) then
          ...
          if (pco%res%y == "y") then
            write (2542,101) time%mo, time%day_mo, time%yrc, ob(iob)%name, res_wat_y(j), res(j)%flo, &
                res_in_y(j)%flo, res_out_y(j)%flo
              if (pco%csvout == "y") then
...
!!!!! average annual print
        if (time%end_sim == 1 .and. pco%res%a == "y") then
          ...
          write (2543,101) time%mo, time%day_mo, time%yrc, ob(iob)%name, res_wat_a(j), res(j)%flo, &
            res_in_a(j)%flo, res_out_a(j)%flo
          if (pco%csvout == "y") then
