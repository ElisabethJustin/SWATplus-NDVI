      ...
      real :: rchvol
      integer :: i
      integer :: iob_ru   !!EB route_seep
      integer :: iob_aqu   !!EB route_seep
      real :: time_peak   !!EBtimerout
      ...
      peakrate = 2. * ht1%flo / (time_peak * 60.)     !sd_chd(isd_db)%tc)
      ...
      if (peakrate > 1.e-9) then
         !! compute changes in channel dimensions
          ...
          !! OVERBANK FLOOD
            ...
            ivalint = 1
            tbase = time_peak * 3600.     !sd_chd(isd_db)%tc * 60.   !hydrograph base time in s
            tb_pr = tbase
            ...
            tbase = time_peak * 3600.     !sd_chd(isd_db)%tc * 60.  !seconds
      ...
      if (rcharea > 1.e-4 .and. ht1%flo > 1.e-4) then
        vc = peakrate / rcharea
        !!vc = 0.8 * 3600  !!EB test influence vitesse (en m/h)
        !if (vc > sd_ch_vel(ich)%celerity_bf) vc = sd_ch_vel(ich)%celerity_bf
        !!EB jhyd n est pas defini rttime = sd_ch(jhyd)%chl * 1000. / (3600. * vc)
      ...
      !!EB route_seep
      iob_aqu = ru_def(ob(icmd)%ru(1))%iob_aqu
      if (iob_aqu>0) then
        ob(iob_aqu)%supply%flo = 10000. * ch_wat_d(ich)%seep  ! en m3
      else
        ch_wat_d(ich)%seep = 0.
      end if
      ...
      !!EBtest2810 deb   prelevement riviere pour irrigation (max 50% du debit actuel)
      !if (ht2%flo > 1) then
      !  rto=ob(icmd)%demand/ht2%flo
      !  if (rto > 0.5) then
      !    !! une partie de la demande est reportee sur le lendemain
      !    rto=0.5
      !    ob(icmd)%demand = ob(icmd)%demand - 0.5*ht2%flo
      !  else
      !    ob(icmd)%demand = 0.   !! reinitialisation de la demande
      !  end if
      !  ht2 = (1 - rto) * ht2    !! prelevement irrigation
      !end if
      !!EBtest2810 fin
      ...
      !else
        ...
        !!EB ch_stor(ich) = frac * ht2 + ch_stor(ich)  !!EB 25/07 ajout +ch_stor  # 
        !!EB ht2 = scoef * ht2    !ht11 -> ht2 on 1-25-2021  ***jga
        ht1 = ht2
        ht2 = scoef * (ht1 + ch_stor(ich))
        ch_stor(ich) = ht1 - ht2 + ch_stor(ich)
        !ht2 = ht2 + ch_stor(ich)
