...
!!    hydrologic cycle
      ...
      if (nd_30 > 30) nd_30 = 1
        ...
        ob(icmd)%hin_lat%flo = ob(icmd)%hin_lat%flo + ob(icmd)%supply%flo/10/ob(icmd)%area_ha   !!EBroute_revap comme un latq finalement
        ob(icmd)%supply = hz
        ...
        if (sched(isched)%num_autos > 0) then
          ...
          do iauto = 1, sched(isched)%num_autos
            ...
            do iac = 1, dtbl_lum(id)%acts
              !!EBif (pcom(j)%dtbl(iauto)%days_act(iac) > 0) then        !!sinon on ne commence jamais la somme...
        ...
        if (hru(j)%ndvi%util) then
          call ndvi_jour   !!EBndvi  met a jour ndvi_j
        end if
        call et_pot
        call et_act
        ...
        inflpcp = precip_eff - surfq(j) !!EB déjà mis dans percmain et déjà converti en mm + hru(j)%water_seep /(10.* hru(j)%area_ha)
...
!!EB sinon c'est comme si la plante n'etait plus stressee dans le fichier resultat  .and. pcom(j)%plcur(ipl)%phuacc <= 1.) then
            npl_gro = npl_gro + 1
