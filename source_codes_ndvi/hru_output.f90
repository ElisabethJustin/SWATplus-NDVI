      ...
      !! daily print
         if (pco%day_print == "y" .and. pco%int_day_cur == pco%int_day) then
          if (pco%wb_hru%d == "y") then
             ...
             !!EB write (2000,100) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hwb_d(j)   !! waterbal
             write (2000,100) time%mo, time%day_mo, time%yrc, ob(iob)%name, hwb_d(j)%et, hwb_d(j)%eplant, hwb_d(j)%esoil, hwb_d(j)%pet, hwb_d(j)%irr    !! waterbal
             if (pco%csvout == "y") then
               !! changed write unit below (2004 to write file data)
               write (2004,'(*(G0.3,:","))') time%mo, time%day_mo, time%yrc, ob(iob)%name, hwb_d(j) !! waterbal !!EB
             end if
          ...
          if (pco%pw_hru%d == "y") then
            !! EB write (2040,101) time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hpw_d(j)  !! plant weather 
            write (2040,101) time%mo, time%day_mo, time%yrc, ob(iob)%name, hpw_d(j)%lai, hpw_d(j)%strsw, hpw_d(j)%phubase0  !! plant weather 
              if (pco%csvout == "y") then 
                !!EB write (2044,'(*(G0.3,:","))') time%day, time%mo, time%day_mo, time%yrc, j, ob(iob)%gis_id, ob(iob)%name, hpw_d(j)  !! plant weather
                write (2044,'(*(G0.3,:","))') time%mo, time%day_mo, time%yrc, ob(iob)%name, hpw_d(j)  !! plant weather
              end if 
          end if
        end if
        ...
        if (time%end_mo == 1) then
          ...
          !! monthly print
           if (pco%wb_hru%m == "y") then
             ...
             write (2001,100) time%mo, time%day_mo, time%yrc, ob(iob)%name, hwb_m(j)%et, hwb_m(j)%eplant, hwb_m(j)%esoil, hwb_m(j)%pet, hwb_m(j)%irr !!EB
               if (pco%csvout == "y") then
           ...
           if (pco%pw_hru%m == "y") then
             write (2041,101) time%mo, time%day_mo, time%yrc, ob(iob)%name, hpw_m(j)%lai, hpw_m(j)%strsw, hpw_m(j)%phubase0  !!EB
               if (pco%csvout == "y") then 
        ...
        if (time%end_yr == 1) then
          ...
          !! yearly print
           if (time%end_yr == 1 .and. pco%wb_hru%y == "y") then
             ...
             write (2002,100) time%mo, time%day_mo, time%yrc, ob(iob)%name,hwb_y(j)%et, hwb_y(j)%eplant, hwb_y(j)%esoil,  hwb_y(j)%pet,  hwb_y(j)%irr
               if (pco%csvout == "y") then
           ...
           if (time%end_yr == 1 .and. pco%pw_hru%y == "y") then
             write (2042,101) time%mo, time%day_mo, time%yrc, ob(iob)%name, hpw_y(j)%lai, hpw_y(j)%strsw, hpw_y(j)%phubase0
               if (pco%csvout == "y") then 
...
!!!!! average annual print
         if (time%end_sim == 1 .and. pco%wb_hru%a == "y") then
           ...
           write (2003,100) time%mo, time%day_mo, time%yrc, ob(iob)%name, hwb_a(j)%et, hwb_a(j)%eplant, hwb_a(j)%esoil,  hwb_a(j)%pet, hwb_a(j)%irr
           if (pco%csvout == "y") then
         ...
         if (time%end_sim == 1 .and. pco%pw_hru%a == "y") then     
           ...
           write (2043,102) time%mo, time%day_mo, time%yrc, ob(iob)%name, hpw_a(j)%lai, hpw_a(j)%strsw, hpw_a(j)%phubase0
             if (pco%csvout == "y") then 
...
100   format (2i3,i5,1x,a10,5f6.1)
101   format (2i3,i5,1x,a10,3f6.3)
102   format (3i6,2x,a,7f10.3)
