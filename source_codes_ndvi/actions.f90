      ...
      use hru_module, only : hru, cn2, fertno3, fertnh3, fertorgn, fertorgp, fertsolp,   &
        ihru, isol, ndeat, phubase, sol_sumno3, sol_sumsolp  !!EB ipl
      use soil_module
      use plant_module
      ...
      do iac = 1, d_tbl%acts
        ...
        if (action == "y") then
          ...
          case ("irr_demand")
            if (ob_cur==50000) write(*,*) '    Jour',time%day,' irr_demand ', d_tbl%act(iac)%option
            ipl = 1
            j = d_tbl%act(iac)%ob_num
            if (j == 0) j = ob_cur

            irrop = d_tbl%act_typ(iac)      ! irrigation application type in irr.ops
            !!EB irrig(j)%demand = d_tbl%act(iac)%const * hru(j)%area_ha * 10.       ! m3 = mm * ha * 10.
            irrig(j)%demand = irrop_db(irrop)%amt_mm * hru(j)%area_ha * 10.   !!EB 
            
            !! if unlimited source, set irrigation applied directly to hru
            if (d_tbl%act(iac)%file_pointer == "unlim") then
              irrig(j)%applied = irrig(j)%applied + irrop_db(irrop)%amt_mm * irrop_db(irrop)%eff * (1. - irrop_db(irrop)%surq)
              irrig(j)%runoff = irrig(j)%runoff + irrop_db(irrop)%amt_mm * irrop_db(irrop)%eff * irrop_db(irrop)%surq
              if (ob_cur==50000) write(*,*) '  irrigation reelle ', irrig(j)%applied  !!EB
              !set organics and constituents from irr.ops ! irrig(j)%water =  cs_irr(j) = 
              pcom(j)%dtbl(idtbl)%days_act(iac) = 0     !reset days since last action  !!EB
          ...
          !irrigate - hru action
          case ("irrigate")
            if (ob_cur==50000) write(*,*) '    Jour',time%day,' irrigation ', d_tbl%act(iac)%option
            ipl = 1
            j = ob_cur                      ! hru number 
            ...
            case ("out")   !!EB eau prise en dehors du BV
              irrig(j)%applied = irrig(j)%applied + irrop_db(irrop)%amt_mm * irrop_db(irrop)%eff * (1. - irrop_db(irrop)%surq) !!EB
              if (ob_cur==50000) write(*,*) '  irrigation reelle ', irrig(j)%applied
              irrig(j)%runoff = irrig(j)%runoff + irrop_db(irrop)%amt_mm * irrop_db(irrop)%eff * irrop_db(irrop)%surq
              pcom(j)%dtbl(idtbl)%days_act(iac) = 0     !reset days since last action
            ...
            !! EB deb Rq: probleme s'il y a des LSU : la upslope n'a pas de channel (mais rq la upslope n'est souvent pas irriguée)
              if (d_tbl%act(iac)%ob_num == 0) then
                iru = sp_ob1%ru + ob(hru(ob_cur)%obj_no)%ru(1)-1  ! numero d'objet du ru
                iob = ob(iru)%obtypno_out(1) 
                if ( ob(iru)%obtyp_out(1) == "res") then  !!EB si reservoir, il n'y a pas de channel => prendre dans le channel en sortie du res
                  iob = ob(sp_ob1%res + iob -1)%obtypno_out(1)
                end if
              else
                iob = d_tbl%act(iac)%ob_num
              end if
              objno_chan = sp_ob1%chandeg + iob -1   ! objno_chan est le no d'objet du channel qu'on veut (le channel dans le mm SBV que la HRU)
            
              if (ob(objno_chan)%src_tot == 0) then
                iob2= 1 
              else
                iob2 = ob(objno_chan)%obtypno_out(1)   ! le no du channel suivant
              end if
              objno_chan2 = sp_ob1%chandeg + iob2 -1    ! objno_chan est le no d'objet du channel suivant

              vol_avail = ob(objno_chan)%hd(1)%flo * 0.75 - ob(objno_chan)%demand

              if (vol_avail > 0.) then     ! on peut prélever dans le channel
                if (irrig(j)%demand < vol_avail-0.01) then    ! prelevement = en entier dans le channel voulu
                  rto = irrig(j)%demand / ob(objno_chan)%hd(1)%flo   ! ratio of water removed from channel volume
                  reste = 0.
                else
                  rto = vol_avail / ob(objno_chan)%hd(1)%flo
                  reste = irrig(j)%demand - vol_avail
                end if
                irrig(j)%water = rto * ob(objno_chan)%hd(1) 
                ob(objno_chan)%demand = ob(objno_chan)%demand + irrig(j)%demand - reste
                cs_irr(iob) = rto * ch_water(iob)                                       ! constituents in irrigation water
                !!EBchange ch_stor(iob) = (1. - rto) * ch_stor(iob)                     ! remainder stays in channel
                !!EBchange ch_water(iob) = (1. - rto) * ch_water(iob)                      ! remainder stays in channel 
              else
                reste = irrig(j)%demand
              end if 
              
              if (reste > 0.) then   ! le reste de la demande est preleve dans le channel suivant
                vol_avail = ob(objno_chan2)%hd(1)%flo * 0.75 - ob(objno_chan2)%demand
              
                if (vol_avail > 0.) then
                  if (reste < vol_avail-0.01) then    ! prelevement = demand dans le channel voulu
                    rto2 = reste / ob(objno_chan2)%hd(1)%flo   ! ratio of water removed from channel volume
                    reste2 = 0.
                  else
                    rto2 = vol_avail / ob(objno_chan2)%hd(1)%flo
                    reste2 = reste - vol_avail
                  end if
                  irrig(j)%water = irrig(j)%water + rto2 * ob(objno_chan2)%hd(1)
                  ob(objno_chan2)%demand = ob(objno_chan2)%demand + reste - reste2
                  cs_irr(iob2) = rto2 * ch_water(iob2)                                         ! constituents in irrigation water
                  !!EBchange ch_stor(iob2) = (1. - rto2) * ch_stor(iob2)                        ! remainder stays in channel
                  !!EBchange ch_water(iob2) = (1. - rto2) * ch_water(iob2)                      ! remainder stays in channel
                else
                  reste2 = reste
                end if 
                
              end if
              irrig(j)%demand = irrig(j)%demand - reste2
              irrig(j)%applied = irrig(j)%applied + irrig(j)%demand / hru(j)%area_ha / 10. * irrop_db(irrop)%eff * (1. - irrop_db(irrop)%surq)
              if (ob_cur==50000) write(*,*) '  irrigation reelle ', irrig(j)%applied
              irrig(j)%runoff = irrig(j)%runoff + irrig(j)%demand / hru(j)%area_ha / 10. * irrop_db(irrop)%eff * irrop_db(irrop)%surq 
              
              if (reste2 < irrop_db(irrop)%amt_mm * hru(j)%area_ha * 10. *0.2) then   ! on a pu satisfaire +de 80% de la demande
                pcom(j)%dtbl(idtbl)%days_act(iac) = 0     !reset days since last action
              else
                if (reste2 < irrop_db(irrop)%amt_mm * hru(j)%area_ha * 10. *0.8)  then    ! on a pu satisfaire entre 20 et 80% de la demande
                  pcom(j)%dtbl(idtbl)%days_act(iac) = 4     !don't totally reset days since last action
                end if
              end if
          ...
          case ("allocate_wro")
            ...
            case ("res")
              ...
              irrig(j)%applied = irrig(j)%applied + irr_mm * irrop_db(irrop)%eff * (1. - irrop_db(irrop)%surq)
              irrig(j)%runoff = irrig(j)%runoff + irr_mm * irrop_db(irrop)%surq
          ...
          case ("till")
            ...
            if (pcom(j)%dtbl(idtbl)%num_actions(iac) <= Int(d_tbl%act(iac)%const2)) then
              ...
              pcom(j)%dtbl(idtbl)%num_actions(iac) = pcom(j)%dtbl(idtbl)%num_actions(iac) + 1
              pcom(j)%dtbl(idtbl)%days_act(iac) = 0     !reset days since this action
              if (iac > 1) pcom(j)%dtbl(idtbl)%days_act(iac-1) =  0     !reset previous action day counter
            end if
          ...
          case ("plant")
            ...
            if (j == 0) j = ob_cur
            icom = pcom(j)%pcomdb
            !!EB ligne deplacee pcom(j)%days_plant = 1       !reset days since last planting
            ...
            if (pcom(j)%dtbl(idtbl)%num_actions(iac) <= Int(d_tbl%act(iac)%const2)) then
              !!EB deb do ipl = 1, pcom(j)%npl
              if (time%day < 210) then
                ipl = mod(pcom(j)%rot_yr-1, pcom(j)%npl) + 1   !! on peut planter des cultures d'ete de l'annee en cours
              else
                ipl = mod(pcom(j)%rot_yr, pcom(j)%npl) + 1   !! on plante des cultures d'hiver de l'annee suivante
              end if  !!EB fin
                idp = pcomdb(icom)%pl(ipl)%db_num
                if (d_tbl%act(iac)%option == pcomdb(icom)%pl(ipl)%cpnm) then
                  if (j==50000) write (*,*) '    Jour',time%day,' plant ',d_tbl%act(iac)%option  !!EB
                  do iplll = 1, pcom(j)%npl
                    call mgt_killop (j, iplll)   !!EB
                  end do
                  pcom(j)%days_plant = 1       !reset days since last planting  !!EB ligne deplacee
                  pcom(j)%plcur(ipl)%gro = "y"
                  pcom(j)%plcur(ipl)%idorm = "n"
                  if (d_tbl%act_app(iac) > 0) then
                    if (j==50000) write (*,*) '    Jour',time%day,' attention call transplant'  !!EB
          ...
          case ("harvest")
            ...
            if (pcom(j)%dtbl(idtbl)%num_actions(iac) <= Int(d_tbl%act(iac)%const2)) then
              ...
              !!EB do ipl = 1, pcom(j)%npl
              ipl = mod(pcom(j)%rot_yr-1,pcom(j)%npl)+1  !!EB
                biomass = pl_mass(j)%tot(ipl)%m
                if (d_tbl%act(iac)%option == pcomdb(icom)%pl(ipl)%cpnm .or. d_tbl%act(iac)%option == "all") then
                  if (j==50000) write (*,*) '    Jour',time%day,' harvest ',d_tbl%act(iac)%option        
