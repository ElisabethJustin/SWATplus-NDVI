...
!      end if
      ...
      !! for each hru or defining unit, set all subbasins that contain it 
        ...
        !!EB route_revap route_seep initialise le no de ru des aqu, res, cha
        do i = 1, sp_ob%aqu
          allocate(ob(i+sp_ob1%aqu-1)%ru(1))
          ob(i+sp_ob1%aqu-1)%ru(1) = 1  !!EB par securité ou pour cas complexes; normalement chaque aqu recoit la perco d'un bassin
        end do
        do i = 1, sp_ob%res
          allocate(ob(i+sp_ob1%res-1)%ru(1))
          ob(i+sp_ob1%res-1)%ru(1) = 1  !! juste par securité; normalement chaque res est dans un RU
        end do
        do i = 1, sp_ob%chandeg
          allocate(ob(i+sp_ob1%chandeg-1)%ru(1))
          ob(i+sp_ob1%chandeg-1)%ru(1) = 1  !!EB juste par securité; normalement chaque channel est dans un RU
        end do   
        ...
        do i = 1, sp_ob%objs
          do ii = 1, ob(i)%src_tot
            ...
            case ("aqu")   !aquifer
              ...
              ob(j)%rcv_tot = ob(j)%rcv_tot + 1
              if (ob(i)%typ == "ru") then
                ru_def(i + 1 - sp_ob1%ru)%iob_aqu = j   !!EB route_seep permet d'avoir accès rapidement a aquifer
                ob(j)%ru(1) = i+1-sp_ob1%ru     !!EB route_revap pour savoir dans quel ru on met le revap
              end if
            case ("cha")   !channel
            ...
            case ("res")   !reservoir
              ...
              ob(j)%rcv_tot = ob(j)%rcv_tot + 1
              if (ob(i)%typ == "ru") then
                ob(j)%ru(1) = i+1-sp_ob1%ru     !!EB route_seep pour savoir dans quel aqu on met le seep du reservoir
              end if
            ...
            case ("sdc")   !swat-deg channel
              ...
              ob(j)%rcv_tot = ob(j)%rcv_tot + 1
              if (ob(i)%typ == "ru") then
                ob(j)%ru(1) = i+1-sp_ob1%ru     !!EB route_seep pour savoir dans quel aqu on met le seep du channel
              end if
            end select
