      ...
      aqu_d(iaq)%rchrg = (ob(icmd)%hin%flo + ob(icmd)%supply%flo) / (10. * ob(icmd)%area_ha)  !!EB ajout 'supply' pour ce qui vient de channel+reservoir
      ob(icmd)%supply = hz
      ...
      aqu_d(iaq)%stor = aqu_d(iaq)%stor - aqu_d(iaq)%seep  !!EB21112022 
      
      !!EB deplace co v60.5.4 !! compute groundwater depth from surface
      ...
      !!EB21112022 !! compute seepage through aquifer and subtract from storage
      !!aqu_d(iaq)%seep = aqu_d(iaq)%rchrg * aqu_prm(iaq)%seep
      !!aqu_d(iaq)%seep = amin1 (aqu_d(iaq)%seep, aqu_d(iaq)%stor)
      !!ob(icmd)%hd(2)%flo = 10. * aqu_d(iaq)%seep * ob(icmd)%area_ha
      ...
      !!EB route_revap mettre le revap dans SWC du ru au-dessus
      iob_ru = sp_ob1%ru + ob(icmd)%ru(1) -1
      ob(iob_ru)%supply%flo = ob(iob_ru)%supply%flo + aqu_d(iaq)%revap * 10. * ob(icmd)%area_ha  !conversion en m3

      !!EB mis a jour 2 fois :) NON deplacé plus haut co v60.5.4 !! compute groundwater depth from surface
      aqu_d(iaq)%dep_wt = aqudb(iaqdb)%dep_bot - (aqu_d(iaq)%stor / (1000. * aqu_prm(iaq)%spyld))
      aqu_d(iaq)%dep_wt = amax1 (0., aqu_d(iaq)%dep_wt)
